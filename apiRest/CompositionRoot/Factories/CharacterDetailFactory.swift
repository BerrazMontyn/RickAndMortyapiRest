...
protocol CharacterDetailFactory {
    func makeModule(coordinator: CharacterDetailViewControllerCoordinator) ->
    UIViewController
...
struct CharacterDetailFactoryImp : CharacterDetailFactory {
    ...
    UIViewController {
        ...
        let loadCharacterDetailUseCase = LoadCharacterDetailUseCaseImp(
            characterDetailRepository: characterDetailRepository, urlDetail: urlDetail)
        let viewModel = CharacterDetailVieModelImp(
            state: state,
    ...
    func makeOriginCoordinator(navigation: UINavigationController) -> Coordinator {
        ...
        let originCoordinator = OriginCoordinator(navigation: navigation, originFactory: originFactory)
        return originCoordinator
    }
    
    func makeLocationCoordinator(navigation: UINavigationController) -> Coordinator {
        let locationDetailFactory = LocationDetailFactoryImp()
        let locationDetailCoordinator = LocationDetailCoordinator(navigation: navigation, locationDetailFactory: locationDetailFactory)
